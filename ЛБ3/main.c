...
float TextureVertex[] = {472,472,552+80,472,552+80,552+80,472,592+40};
...
{
    float HeroVertex[] = {obj.x-100,obj.y-80,obj.x+100,obj.y-80,obj.x+100,obj.y+80,obj.x-100,obj.y+80};
...
void Hero_Move(Hero *obj)
{
    ...
    if (GetKeyState(VK_LEFT)<0)
    {
        ...
        obj->dx-=speed;
        obj->x+=obj->dx;
        Reflect(&obj->dx, &obj->x, (obj->x-50 < 0), 50);
        obj->dx=0;
    ...
    if (GetKeyState(VK_RIGHT)<0)
    {
        ...
        obj->dx+=speed;
        obj->x+=obj->dx;
        Reflect(&obj->dx, &obj->x, (obj->x+50 > width), width-50);
        obj->dx=0;
        if (currentframe>8) currentframe-=7;
    }
    if (GetKeyState(VK_UP)<0 && (obj->y>height-300))
    {
        TypeOfAnim=1;
    ...
    if(!(GetKeyState(VK_UP)<0) && (obj->y<height-160))
    {
...
{
    ChooseBTN(0,2);
    Menu_AddButton("Begin",20,20,200,60,4);
    Menu_AddButton("Quit",220,20,200,60,4);
    Menu_AddButton("Back",20,20,200,60,4);
}
...
void InitGame()
{
    Hero_Init(&Robo,width/2,height-160,0.5,0.5);
    ChooseBTN(2,3);
    currentframe=0;
    TypeOfAnim=0;
    anim=1;
...
void MouseDown()
{
    ...
    if (strcmp(name,"Begin")==0) InitGame();
                   ...
                   int nCmdShow)
{
    ...
    Texture_LoadTexture("CyberChel.png",&textureId,GL_REPEAT,GL_NEAREST, NULL);
    ...
    while (!bQuit)
    {
        ...
        else
        {
            ...
            glClearColor(0.2f, 0.8f, 0.75f, 0.0f);
